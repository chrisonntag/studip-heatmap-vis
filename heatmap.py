...
# Code below is copied from this StackOverflow comment
...
def prepare_heatmap(activity_data):
    ...
    now = datetime.today()
    past = now - timedelta(days=364)

    data = []
    for dt in rrule.rrule(rrule.DAILY, dtstart=past, until=now):
        date_string = str(dt)[:-9] # remove time
        if date_string in activity_data.keys():
            data.append(activity_data[date_string])
        else:
            data.append(0)
    ...
    data = np.array(data)
    data = pd.Series(data)
    data.index = pd.DatetimeIndex(start=str(past), end=str(now), freq='1D')
